    ...
    def generate_final_analysis(self, results: dict, parsed_data: dict = None):
        ...
        self._final_reasoning = _llm_call(
            system=(
                ...
                "vs cudaFuncSetAttribute API, (d) ncu DRAM throughput % "
                ...
                "frequency — NCU uses --clock-control which locks the GPU to "
                "base clock during profiling; clock64 measures the boost clock "
                "under sustained load. If the GPU was frequency-locked (warmup_ratio≈1.0) "
                "these two should AGREE within <2% (base=boost). If the GPU is "
                "naturally boosting, NCU reports base clock (~1395 MHz) while "
                "clock64 reports boost clock (~1946 MHz) — the split is "
                "INFORMATIONAL and confirms genuine boost behavior. "
                "State which case applies and cite the exact MHz values. "
                "State deviations numerically and "
